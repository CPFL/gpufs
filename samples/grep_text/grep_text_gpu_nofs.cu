...
{
    for (int i = threadIdx.x; i < total_buf; i += blockDim.x)
        dst[i] = 0;
}
...
#pragma unroll 32
    for (int i = 0; i < data_size; i++) {
        if (!alpha(data[i])) {
            if ((sizecount == 32 || a[sizecount] == '\0') && word_start) {
                matches++;
                *wordlen = sizecount;
            }
            word_start = 1;
        ...
        for (size_t _cursor = 0; _cursor < db_size;) {
            ...
            for (int input_block = 0; input_block < data_to_process; input_block += INPUT_PREFETCH_SIZE) {
                ...
                if (output_count) {
                    ...
                    if (threadIdx.x == 0)
                        old_offset = atomicAdd(&global_output, output_count);
                    __syncthreads();
